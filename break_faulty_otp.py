...
dc = [bytes.fromhex(i) for i in CIPHERS]

is_space = lambda a: (a>= ord('A') and a <= ord('Z')) or (a >= ord('a') and a <= ord('z'))

xor_bins = lambda a,b: [i^j for i,j in zip(a,b)]

xor_bins_check_space = lambda a,b: [is_space(i^j) for i,j in zip(a,b)] + [False] * (max(len(a), len(b)) - min(len(a), len(b)))

space_hits = lambda a, i : sum([xor_bins_check_space(a, b)[i] for b in dc if b!=a])

hits_sorted = lambda b,i : sorted([(a[i], space_hits(a,i)) for a in dc if a!=b and i < len(a)], key=lambda x: x[1], reverse = True) + [(0,0)]

space_cand = lambda b, i, thresh : hits_sorted(b,i)[0][0] if hits_sorted(b,i)[0][1] > thresh else None

dec = lambda a, thresh: ''.join([chr(a[i] ^ space_cand(a,i,thresh) ^ ord(' ')) if space_cand(a,i,thresh) is not None else '_' for i in range(len(a))])

THRESHOLD = 5

for a in dc:
    print(dec(a,THRESHOLD))
